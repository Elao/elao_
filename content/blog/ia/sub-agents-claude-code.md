...
Moins de contexte, c'est aussi **moins de bruit** pour le modèle. Le sub-agent se concentre sur sa tâche sans être distrait par des informations qui ne le concernent pas. Et accessoirement, moins de tokens en entrée signifie une exécution plus rapide et moins coûteuse.

## Utiliser un modèle différent

Dans une conversation Claude Code, le modèle est fixé au départ. Si vous travaillez avec Opus, vous êtes sur Opus du début à la fin. Il n'est pas possible de basculer sur un autre modèle en cours de route — changer de modèle implique de démarrer une nouvelle conversation, et donc de perdre tout le contexte accumulé.

Les sub-agents permettent de contourner cette limitation. Chaque sub-agent peut être lancé avec un **modèle différent** de celui de l'agent principal, via le paramètre `model`. On peut donc orchestrer une conversation où Opus prend les décisions complexes pendant que des sub-agents Haiku ou Sonnet se chargent des tâches plus simples.
...
L'intérêt n'est pas qu'intellectuel — il est aussi économique. Haiku est **significativement moins cher** qu'Opus en consommation de tokens. Si votre workflow lance 10 sub-agents pour des tâches triviales, les faire tourner sur Haiku plutôt qu'Opus peut diviser le coût de ces sous-tâches par un facteur important.
...
Par défaut, Claude Code exécute tout séquentiellement : une action après l'autre, dans une seule conversation. Si vous avez 10 tâches indépendantes à traiter, il les fera une par une. Pour certaines tâches, c'est parfaitement acceptable. Mais quand chacune prend 30 secondes à une minute, l'addition devient douloureuse.

Les sub-agents permettent de **lancer plusieurs tâches en parallèle**. Le principe est simple : l'agent principal crée plusieurs sub-agents dans un même message, et Claude Code les exécute simultanément. Chaque sub-agent travaille de façon indépendante dans son propre contexte, et l'agent principal récupère les résultats au fur et à mesure.
...
La clé technique : pour que les sub-agents soient lancés en parallèle, il faut demander explicitement à Claude Code de les créer **dans un même message** :

```markdown
Launch one Agent per task, **all in a single message** so they
run concurrently. Use `run_in_background: true` so the main agent
is not blocked and can react to completion notifications.
```

Sans cette précision, Claude Code les lancerait séquentiellement, l'un après l'autre.
...
Les sub-agents n'ont pas accès à tous les outils disponibles dans la conversation principale. Notamment, les outils de gestion de tâches (`TaskList`, `TaskGet`, `TaskUpdate`) ne sont accessibles que par l'agent principal. Si votre sub-agent a besoin de consulter ou mettre à jour une liste de tâches, il ne pourra pas — il faudra que l'agent principal s'en charge.

C'est une contrainte qui impacte directement la façon dont on conçoit ses workflows. On ne peut pas créer un sub-agent autonome qui va piocher tout seul dans une file de tâches. C'est l'agent principal qui doit distribuer le travail et centraliser la coordination.
